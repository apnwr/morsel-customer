...
Systemic resolution strategy for the _"Peach credentials not found in Secret Manager"_ class of errors. Frames the issue as a **tenant-provisioning gap**, not a payment bug, and prescribes a layered fix.

**Audience:** engineering leads, backend, frontend, ops.
**Status:** proposal — drafted 2026-04-18, remains UNIMPLEMENTED as of 2026-05-29 (no `paymentsEnabled` session flag, no structured error codes shipped).
...
| Graceful fallback CTAs | _Pay at counter_ / _Notify staff_ buttons when payments are unavailable. Order is placed; settlement moves offline.            | P1       |
...
Hiding the doomed button is both better UX _and_ cheaper.
...
| Block the whole ordering flow when creds are missing | Lose revenue for no reason. Gate _settlement_, not ordering.                                  |
