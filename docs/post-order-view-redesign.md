...
> **STATUS: SHIPPED 2026-05** — fully implemented and superseded by the deepak-new bill/split flow. `PostOrderView` now lives at `/orders` (not `/cart`); payment results render via `PaymentResultView`; the split/bill/tip sections and sticky Pay Now footer described as [PENDING] below all shipped (`TipSelector` extracted to `src/components/cart/TipSelector.tsx`). Retained for history — do not treat as current/pending work.
>
> **NOTE:** Step 8's planned deletion of `src/app/success/page.tsx` and `src/app/failure/page.tsx` was **NOT** done — those routes still exist (track as separate cleanup).
