...
> **Status: PROPOSAL, do-not-migrate (still standing as of 2026-05-29).** The proposed `/payment-intents`, `/simple-purchase`, `/embedded-checkout`, `/confirm-payment` paths remain absent from `endpoints.ts`.
...
- **Adopting this spec does NOT fix the current _"Peach credentials not found in Secret Manager"_ error.** That error is a data-provisioning issue (see `peach-credentials-issue.md`) and exists regardless of which payment architecture is used.
...
The error _"Peach credentials not found in Secret Manager"_ fires when the backend cannot find `PEACH_PAYMENTS_KEYS` for the business that owns the session. In both architectures, the backend must eventually:
...
3. If we pass `clientId` on the _existing_ `/payments/peach-payments/verify`, what does it do today?
