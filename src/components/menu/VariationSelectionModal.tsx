...
const DietarySymbol = ({
  ...
  <div
    className={`w-3.5 h-3.5 flex items-center justify-center rounded-[2px] border-[1.5px] bg-white shrink-0 ${
      dietaryType === "vegetarian" ? "border-green-600" : "border-red-600"
    }`}
...
export function VariationSelectionModal({
    ...
    cartItems.forEach((item) => {
      ...
      if (groups.has(groupKey)) {
        ...
        const customizationSummary =
          parts.length > 0 ? parts.join(" • ") : "No customizations";
        ...
        groups.set(groupKey, {
          ...
          ownerName: isOwnedByCurrentUser
            ? undefined
            : getParticipantName(item.sessionUserId),
  ...
  // Ownership routing now happens in the parent's onRemove/onDecrement
  // callbacks: own → mutate, others → open OwnershipBlockedSheet.
  const handleDecrementGroup = (
    ...
    if (group.totalQuantity === 1) {
      ...
      // Close modal only when the removal can actually happen (own row).
      // For others' rows the OwnershipBlockedSheet surfaces above instead.
      if (group.isOwnedByCurrentUser && groupedItems.length === 1) {
        onClose();
      }
  ...
  // For others' rows the parent closes the modal and opens the customization
  // modal so the user picks their own variant.
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Adjust quantity">
      <div className="space-y-3">
        {groupedItems.map((group) => {
          ...
          const unitPrice =
            firstItem.menuItem.price +
            firstItem.customizations.reduce(
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm text-gray-600 mt-1">
                  {group.totalQuantity} × {formatPrice(unitPrice)} ={" "}
                  {formatPrice(unitPrice * group.totalQuantity)}
                </div>

                {/* Show owner info for items not owned by current user.
                    Controls below are tappable on these rows; taps surface
                    the OwnershipBlockedSheet (−) or the customization modal (+). */}
                {!group.isOwnedByCurrentUser && (
                  <div className="mt-2 text-xs text-amber-700 font-medium">
                    Added by {group.ownerName}
                  </div>
                )}
              </div>

              {/* Quantity controls render for every row. Ownership routing
                  happens in the parent's onRemove/onDecrement/onIncrement
                  callbacks; this modal stays presentational. */}
